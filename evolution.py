...
class EA:
    ...
    def initialize(self,
        ...
        if INIT_DNA==None:
            ...
            while f:
                ...
                print('Initial DNA: ',pd.DataFrame(INIT_DNA[0],index=self.DNA_IDX,columns=['']),'\n')
                if input('是否重新生成?(y/n) ')=='n':f=False
                else:f=True
        else:
    ...
    def mutate(self,kids):
        ...
        DNA[:] = 1/(1+np.exp(-np.random.randn(*DNA.shape)*MTR-np.log(DNA/(1-DNA))))
    ...
    def main(self):
        ...
        self.EvolutionHistroy = np.vstack((self.EvolutionHistroy,np.mean(self.pop['DNA'],0)))
    ...
    def plot(self):
        ...
        plt.cla()
        plt.axis(self.axis)
        plt.title('进化史（当前为第{}代）'.format(len(self.EvolutionHistroy)),fontproperties='SimHei',size=15)
    ...
    def mainloop(self,N_GENERATIONS=None):
        ...
        if N_GENERATIONS==None:
            ...
            self.axis = [0, N_GENERATIONS+len(self.EvolutionHistroy)-1, -0.1, 1.1]
            self.plot()
